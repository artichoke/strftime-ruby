...
//! to u16::MAX, to improve memory layout and prevent silent truncation bugs on
//! cross-compilation targets. This change caused previously valid dynamic width
//! values exceeding u16::MAX to panic at runtime. (See rust-lang/rust#136932
//! and artichoke/strftime-ruby#2f0ab4a).
//!
...
fn test_rust_136932_reduce_fmt_argument_width_and_precision_fuzzer_failures() {
    let test_cases: [(MockTime<'_>, &str); 8] = [
        ...
        (
            ...
            "%u\0\n\0\0\u{10}\0`\u{31b}@\0\0\02>`\0%33333333s33333#33333333333333333333333333333333333\u{13}33333333333u\0\0\0\0\0%u\u{4}\0\0\0\u{10}\0`"
        ),
    ];
...
fn test_format_specifiers_large_width_success() {
    ...
    // For each, we construct a format string with a width of 65,636.
    // The format string is wrapped in single quotes so that we can easily
    // strip them and check that the inner formatted result has exactly the given width.
    let specifiers = [
        "Y", "C", "y", "m", "B", "b", "d", "e", "j", "H", "k", "I", "l", "P", "p", "M", "S", "L",
        "N", "z", ":z", "::z", ":::z", "Z", "A", "a", "u", "w", "G", "g", "V", "U", "W", "s", "n",
        "t", "c", "D", "F", "v", "r", "R", "T", "X",
    ];
    let width: usize = 65_636;
    ...
    for &spec in specifiers.iter() {
        ...
        result.expect(&format!("Failed for specifier '{spec}' with width {width}"));
...
fn test_format_specifiers_int_max_fail() {
    ...
    // Test that using a width equal to INT_MAX (2,147,483,647) causes an error
    // (e.g. due to write buffer limits). We use a small output buffer so that
    // the formatting attempt cannot succeed.
    let specifiers = [
        "Y", "C", "y", "m", "B", "b", "d", "e", "j", "H", "k", "I", "l", "P", "p", "M", "S", "L",
        "N", "z", ":z", "::z", ":::z", "Z", "A", "a", "u", "w", "G", "g", "V", "U", "W", "s", "n",
        "t", "c", "D", "F", "v", "r", "R", "T", "X",
    ];
    let width: usize = i32::MAX as usize;
    // A valid and interesting MockTime instance that exercises a wide range of
    // specifiers (e.g. year, month, day, time, fractional seconds, week day,
    // time zone, etc.):
    let time = MockTime::new(
